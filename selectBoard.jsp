...
<%!
// DB에연결하기 위한 Connection 클래스 타입의 변수 생성
//Connection conn = null;
// SQL문을 DB 서버에 질의하기 위한 Statement 클래스 타입의 변수 생성
//Statement stmt = null;
...
//	boardList에서 데이터 전송 시 정수 데이터로 전송했으나 request 내장 객체 에서 저장하고 있는 데이터가 문자열 형태 이므로 이를 다시 정수로 변경하기 위해서 parseInt() 메서드를 사용함
...
// 		jdbc 드라이버가 설정되면 자동으로 DriverMarger에 등록되고, 객체가 생성됨 
//		getConnection() 매서드를 상요하여 실제로 DB 서버에 연결함
...
//		받아온 데이 에서 next() 메서드를 사용하여 결과값이 있는 지 여부를 확인
		while (rs.next()) {
//			getInt, getString 메서드를 사용하여 각각의 컬럼에 있는 데이터를 
	...
	%>
		<div class="mt-5">
			...
			<div class="my-3">
				...
				<input type="text" class="form-control" id="userId" name="useId" readonly value="<%= userId %>">
			</div>
			<div class="my-3">
...
<!--				각각의 페이지를 호출하면서 매개변수로 글번호를 넘겨줌  -->
					<a href="updatedBoard.jsp?num=<%= idx %>" class="btn btn-info">수정</a>
